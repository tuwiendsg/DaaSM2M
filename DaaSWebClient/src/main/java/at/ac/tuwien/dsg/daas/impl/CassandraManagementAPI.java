...
public class CassandraManagementAPI implements DataManagementAPI {
	 ...
	 *            usually UUID but can also be string (less performance) or
	 *            anything else
	 * @param columns
	 *            a map containing as Key the column NAME and as value the
	 ...
	 * @param tableName
	 * @param columns
	 *            creates an index on ALLL supplied columns
	 */
	 ...
	 * @param tableName
	 * @param columns
	 *            deletes indexes from ALLL supplied columns
	 */
	 ...
	 * @param tableName
	 * @param condition
	 *            I do not have a better idea at the moment. Aimed at maximum
	 *            freedom, the condition can be any CQL3 statement,
	 *            http://www.datastax.com/docs/1.1/references/cql/SELECT
	 *            NOTE!!!!!!!!!!!!!!!!!!!: FIRST clause in WHERE must include
	 *            the PRIMARY KEY NO <,> (LT,GT) are supported
	 * 
	 *            Example: name='bob' AND score>= 40;
	 * @return a Row if the query returned anything, or null otherwise Can be
	 *         used to select first row by leaving condition null, get Timestamp
	 *         from it, use it in select "where timestamp smaller timestamp of
	 *         retrieved previousely" to get the next event in line, and so on
	 ...
	 *            it limits the Nr. of Rows returned (if 1 row required you can
	 *            specify 1, for example to retrieve first row from database)
	 * @param condition
	 *            I do not have a better idea at the moment.
	 * 
	 *            Aimed at maximum freedom, the condition can be any CQL3
	 *            statement,
	 *            http://www.datastax.com/docs/1.1/references/cql/SELECT
	 *            NOTE!!!!!!!!!!!!!!!!!!!: FIRST clause in WHERE must include
	 *            the PRIMARY KEY NO <,> (LT,GT) are supported
	 * 
	 *            Example: name='bob' AND score = 40 userID = 'some_key_value'
	 *            userID IN (key1, key2)
	 * 
	 * @return a List of max rowCount rows if the query returned anything, or
	 *         null otherwise Can be used to select first row by leaving
	 *         condition null, get Timestamp from it, use it in select "where
	 *         timestamp smaller timestamp of retrieved previousely" to get the
	 *         next event in line, and so on until it returns nothing.
	 * 
	 ...
	 * @param tableName
	 * @param rows
	 *            each row is a map of String,DataType containing
	 */
	 ...
	 * @param tableName
	 * @param rows
	 *            each row is a map of String,Value containing columnName=value
	 *            pairs
	 */
